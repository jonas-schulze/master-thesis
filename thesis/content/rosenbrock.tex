...
\begin{equation}
  ...
  ,
\end{equation}
which causes the stage equations~\eqref{eq:ros:def} to become an \ac{ALE}.
...
\begin{remark}
  When formulating these methods for generalized \acp{DRE},
  \ie replacing
  \begin{align*}
    X &\gets E^\T X E, &
    A &\gets E^{-1} A, &
...
\begin{remark}
  For generalized \acp{DRE} it is $G = \begin{bmatrix}
    C^\T & E^\T L
...
\begin{remark}
  For generalized \acp{DRE} it is $G^{(1)} = \begin{bmatrix}
    C^\T & A^\T L & E^\T L
...
The following construction is based on the work of \citeauthor{Mena2007}~\cite{Mena2007}.\footnote{%
  ...
  which have hopefully all been eliminated for
...
\begin{remark}
  For generalized \acp{DRE} it is $G^{(21)} = E^\T T_1$ and $S^{(21)}$ remains unchanged.
\end{remark}

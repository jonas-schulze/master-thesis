...
% This paragraph is to control page break and indentation.
% If it is removed, join he paragraphs surrounding it.
...
define methods for the \julia{ParaReal.dist()} and \julia{LinearAlgebra.norm()} functions.
...
Golang for example does this by default.\footnote{%
...
\julia{ParaReal.jl} cancels the whole pipeline if a stage fails.
...
\julia{ParaReal.jl} tries 3 times to start a stage, if it didn't fail from user error.
...
This issue, however, is being worked on.\footnote{%

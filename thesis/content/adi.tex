...
\begin{proof}
  %Obvious.
  $H$, $V$, $\gamma I_n$ for $\gamma\in\C$, and thus $\Aipm,\Aiipm$ commute $\forall k\in\N$.
\end{proof}
...
\label{footnote:adi:transpose}
  ...
  Both variants are common and, hopefully, won't cause confusion.
...
\begin{remark}
  ...
  This leads to the impression that the ADI is a single-parameter ADI having $\tilde\beta_k := \tilde\alpha_k$
  instead of being a true two-parameter ADI where potentially $\alpha_k \neq \beta_k := \conj{\alpha_k}$.
...
one may replace the spectral norm in \eqref{eq:adi:residual} and \cite[Algorithm~2.2]{Lang2017}
...
\begin{algorithm}[tp]
  ...
  \Repeat{$\norm[\big]{R_{k-1}^\T R_{k-1} S}_F \leq \epsilon \norm[\big]{G^\T GS}_F$}{%
  %TODO: ...or $j > J$
    ...
    \eIf{$\alpha_k \in\R$}{%
      ...
      $D_k \gets \begin{bmatrix}
        ...
        & Y_k
      \end{bmatrix}$\;
      $R_k \gets R_{k-1} - 2\alpha_k E V_k$\;
      $k \gets k+1$\;
    }{%
      ...
      $D_{k+1} \gets \begin{bmatrix}
        ...
        && Y_k
      \end{bmatrix}$\;
      $R_{k+1} \gets R_{k-1} - 4\Re(\alpha_k) E \big( \Re(V) + \delta_k \Im(V) \big)$\;
      $ k \gets k + 2$\;
    }

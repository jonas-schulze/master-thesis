...
\begin{align}
  \text{stage } n &
  \ldots, G(U_{n-1}^k), U_n^k, F(U_{n-1}^k), \ldots
  \nonumber\\
  \text{stage } n+1 & \enspace
  \ldots, G(U_{n}^k), U_{n+1}^k, F(U_{n}^k), \ldots
  \text{stage } n+2 & \quad
  \ldots, G(U_{n}^k), U_{n+1}^k, F(U_{n}^k), \ldots
  \nonumber
\end{align}
...
\begin{equation}
  \begin{tikzpicture}[
    ...
    \graph[grow right=2cm] {
      ""/"stage $n+1$: \ldots" -!- Gnk1/"$G(U_n^{k+1})$," -!- Un1k1/"$U_{n+1}^{k+1}$," -!- Fnk/"$F(U_n^{k+1})$," -!- ""/"\ldots"
    };
    \draw[<-] (Gnk1.south) -- +(-120:5mm);
    \draw[->] (Un1k1.north) -- +(60:5mm);
  \end{tikzpicture}
\end{equation}
